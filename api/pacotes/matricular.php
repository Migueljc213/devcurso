...
$query = $pdo->query("SELECT * FROM pacotes where id = '$curso'");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$valor = $res[0]['valor'];
...
$res = $query->fetchAll(PDO::FETCH_ASSOC);
if(@count($res) > 0){
	$result = json_encode(array('mensagem'=>'Aluno já Matriculado no Pacote!', 'sucesso'=>false));
	echo $result;
	exit();
}else{

	if($valor == '0'){
		$pdo->query("INSERT INTO matriculas SET id_curso = '$curso', aluno = '$id_aluno', professor = '$professor', valor = '$valor', data = curDate(), status = 'Matriculado', pacote = 'Sim', subtotal = '$valor', aulas_concluidas = '1'");
	}else{
		$pdo->query("INSERT INTO matriculas SET id_curso = '$curso', aluno = '$id_aluno', professor = '$professor', valor = '$valor', data = curDate(), status = 'Aguardando', pacote = 'Sim', subtotal = '$valor', aulas_concluidas = '1'");
	}
